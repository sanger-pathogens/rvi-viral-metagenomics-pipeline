...
include { VRHYME_BIN; VRHYME_BIN_MULTISAMPLE; VRHYME_BIN_FAIRY } from './rvi_toolbox/subworkflows/vrhyme.nf'
...
workflow {
    ...
    if (params.vrhyme_use_fairy_coverage) {
        // Cross-sample bowtie → fairy → reformat → vRhyme -c
        VRHYME_BIN_FAIRY(
            GENOMAD_CLASSIFY.out.virus_fna,
            GENOMAD_CLASSIFY.out.virus_summary,
            ready_reads_ch
        )
    } else if (params.vrhyme_multisample_coverage) {
        // Cross-sample bowtie → vRhyme -b (multiple BAMs)
        VRHYME_BIN_MULTISAMPLE(
            GENOMAD_CLASSIFY.out.virus_fna,
            GENOMAD_CLASSIFY.out.virus_summary,
            ready_reads_ch
        )
    } else {
        // Per-sample bowtie → vRhyme -b (single BAM)  (default)
        VRHYME_BIN(
            GENOMAD_CLASSIFY.out.virus_fna,
            GENOMAD_CLASSIFY.out.virus_summary,
            ready_reads_ch
